...
nonisolated struct EditFileInput: Decodable, Sendable {
  let path: String
  let oldText: String
  let newText: String
...
nonisolated struct EditFileToolExecutor: TypedToolExecutor {
  static let definition = ToolDefinition.editFile
  ...
  func previewApproval(_ input: EditFileInput, context: ToolContext) async -> ToolResultPreview? {
    do {
  ...
  func run(_ input: EditFileInput, context: ToolContext) async -> ToolResultPreview {
    do {
      return try context.workspace.withSecurityScopedAccess {
        let edit = try validatedEdit(input, context: context)
...
nonisolated enum EditFileValidationError: LocalizedError {
  case emptyOldText
