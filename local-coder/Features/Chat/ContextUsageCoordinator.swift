...
nonisolated struct ContextUsageSnapshot: Sendable {
...
nonisolated enum ContextUsageEvent: Sendable, Equatable {
  case reset
...
@MainActor
final class ContextUsageCoordinator {
  ...
  func invalidate(onEvent: @escaping @MainActor (ContextUsageEvent) -> Void) {
    requestID = UUID()
    cancel()
    onEvent(.reset)
  }
