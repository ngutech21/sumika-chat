...
struct GemmaMLXRuntimeTemplateTests {
  ...
  @Test
  func cacheDecisionReportsRangeContextMismatchSeparatelyFromBasePrompt() {
    let settings = ChatGenerationSettings.codingDefault
    let prefix = [
      GemmaMessageSnapshot(
        role: "user",
    ...
    let changedSelectedRange = [
      GemmaMessageSnapshot(
        ...
        content: """
          ...
          Selected file range: Sources/Bar.swift
          Lines: 1-3
          Selected content excerpt:
          let value = 2
          Explicit file paths in the user request or tool call take precedence.
    ...
    let changedVisibleRange = [
      GemmaMessageSnapshot(
        ...
        content: """
          ...
          Visible file range: Sources/Bar.swift
          Lines: 4-9
    ...
    let selectedDecision = GemmaMLXRuntime.cacheDecision(
      cachedPrefix: prefix,
      cachedSettings: settings,
      cachedState: .clean,
      currentHistory: changedSelectedRange,
      currentSettings: settings
    )
    let visibleDecision = GemmaMLXRuntime.cacheDecision(
      cachedPrefix: prefix,
      cachedSettings: settings,
      cachedState: .clean,
      currentHistory: changedVisibleRange,
      currentSettings: settings
    )
