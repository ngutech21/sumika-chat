...
struct DataModelMarkdownRendererTests {
  ...
  func rendererProducesDeterministicMarkdownWithRelations() {
    let document = DataModelDocument(
      models: [
        ...
        DataModelDeclaration(
          name: "ChatMessage",
          kind: .struct,
          sourcePath: "Sources/LocalCoderCore/Models/ChatMessage.swift"
        ),
        DataModelDeclaration(
          name: "ReadFileResult",
          kind: .enum,
    ...
    #expect(markdown.contains("  ChatSessionState --> ChatMessage"))
    ...
    #expect(markdown.contains("- `messages: [ChatMessage]`"))
